...
function markPath(path: string[], wireName: string): void {
	...
	path.map(step => {
		...
		moveInDirectionFrom(currentPoint, direction, length, wireName);
		console.log("after moving", direction, length, currentPoint);
	});
}

function moveInDirectionFrom(point: Point, direction: Direction, length: number, wireName: string): void {
	while (length > 0) {
		switch (direction) {
			case "U": 
				point.y = point.y + 1;
		...
		if (!existingWires) {
			coordinatesMap.set(indexCoord, [wireName + "," + (length !== 1 ? direction : "X")]);
		} else if (areWiresCrossing(existingWires[0], wireName, direction)) {
			coordinatesMap.set(indexCoord, [...existingWires, wireName + "," + (length !== 1 ? direction : "X")]);
			console.log("Just set", indexCoord, coordinatesMap.get(indexCoord));
		}
...
function areWiresCrossing(existingWire: string, newWire: string, newDirection: Direction): boolean {
	const [existingWireName, existingDirection] = existingWire.split(",");

	if (existingWireName === newWire) {
		return false;
	}
	...
	switch(existingDirection) {
		case "U": 
		case "D": 
			return newDirection === "L" || newDirection === "R"
		case "R": 
		case "L": 
			return newDirection === "U" || newDirection === "D"
	}
}

function smallest(a: number[]): number {
